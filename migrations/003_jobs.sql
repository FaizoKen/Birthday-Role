...
--   * 'player_sync'  → {"discord_id": "..."}            (a member set/changed their birthday)
--   * 'config_sync'  → {"guild_id": "...", "role_id": "..."}  (rule changed, or the daily tick)
--
...
-- index keeps the working set tiny even with many completed rows.
...
-- Detect stuck-in-progress rows (worker crashed after claiming). The reaper
-- flips these back to pending after a TTL.
CREATE INDEX IF NOT EXISTS idx_jobs_locked
    ON jobs (locked_at)
    WHERE status = 'in_progress';

-- The tick worker debounces "resync everything" by deduplicating an
-- already-pending config_sync for the same (guild_id, role_id). This index
-- makes that existence check an index probe instead of a scan.
CREATE INDEX IF NOT EXISTS idx_jobs_pending_dedupe
    ON jobs (kind, ((payload->>'guild_id')), ((payload->>'role_id')))
    WHERE status = 'pending';
