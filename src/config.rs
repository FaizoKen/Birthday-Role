...
pub struct AppConfig {
    ...
    /// Shared HMAC key — verifies the Auth Gateway's `rl_session` cookie and
    /// signs the iframe-session token. MUST equal the gateway's value.
    pub session_secret: String,
    /// Public base URL including the `/birthday-role` path prefix.
    pub base_url: String,
    ...
    /// Origin allowed to embed the role-config page in an iframe. Builds the
    /// `Content-Security-Policy: frame-ancestors …` header. Unset → falls
    /// back to the production dashboard origin.
    ...
    /// time-relative rules flip on schedule. Clamped to a 5-minute floor.
    ...
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            worker_concurrency: env::var("WORKER_CONCURRENCY")
                ...
                .and_then(|v| v.parse().ok())
                .unwrap_or(4),
            tick_minutes: env::var("TICK_MINUTES")
                .ok()
