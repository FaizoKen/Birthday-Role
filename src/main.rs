...
pub struct AppState {
    ...
    /// Shared HTTP client for Auth Gateway calls (sync workers + admin
    /// preview + cookie permission checks).
    pub http: reqwest::Client,
    /// Origins permitted to issue cookie-authenticated state-changing
    /// requests. Source of truth for the `CorsLayer` allowlist and the
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "birthday_role=info,tower_http=warn".into()),
        )
        .init();

    // `migrate` subcommand: apply migrations and exit (blue-green deploys).
    ...
    // Origins that can drive cookie-authenticated state changes. The
    // CorsLayer mirror is built from this same list below.
    ...
    let plugin_routes = Router::new()
        ...
        .route("/verify/save", post(routes::verify::verify_save))
    ...
    // -------- Middleware stack (outermost declared last runs first) --------
    ...
    // Per-IP rate limiter. Burst 20 absorbs UI bursts; sustained 5/s is far
    // above any human usage but chokes floods. SmartIpKeyExtractor reads the
    // forwarded client IP — the reverse proxy MUST overwrite it.
    ...
    let app = Router::new()
        .nest("/birthday-role", plugin_routes)
    ...
    // `into_make_service_with_connect_info` is required for
    // `SmartIpKeyExtractor` to fall back to the peer address.
    ...
    if let Err(e) = tick_handle.await {
        tracing::error!("Tick worker join failed: {e}");
    }
