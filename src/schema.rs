...
//! stale/leaked call can't ping silently.
...
pub fn build_iframe_config(base_url: &str, guild_id: &str, role_id: &str) -> Value {
    ...
    json!({
        ...
        "ui_mode": "iframe",
        "name": "Birthday Role",
        "description": "Automatically grant Discord roles from members' self-reported birthdays — \
                        birthday-of-the-day, birthday week/month, zodiac, season, age and more, \
                        all timezone-aware.",
        "embed_url": embed_url,
    })
}
...
/// chance an older backend forwards a call. Token already verified in the handler.
