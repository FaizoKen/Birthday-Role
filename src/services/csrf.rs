...
//! `Origin` header on every state-changing cookie request and rejects it if
//! the origin isn't on our allowlist. A browser always sets `Origin` on
//! POST/PUT/DELETE from a real page; non-browser callers won't, which is
//! exactly what we want to reject on cookie write paths.
//!
