...
//! replica can pick up a job (`FOR UPDATE SKIP LOCKED`); transient failures
//! retry with exponential backoff + jitter; permanently-failing jobs land in
//! a DLQ (`status = 'dead'`).
//!
//! Enqueue with the typed `enqueue_*` helpers (each INSERTs and conditionally
//! `pg_notify`s in one round-trip); [`crate::tasks::job_worker`] claims and
//! dispatches.
...
/// Postgres NOTIFY channel fired when an immediately-runnable job is
...
/// Delete old terminal rows so the table stays small. Called by the tick
/// worker. Keeps a short window of `dead` rows for DLQ inspection.
pub async fn gc_old(pool: &PgPool) -> Result<u64, AppError> {
    let res = sqlx::query(
        "DELETE FROM jobs \
         WHERE (status = 'completed' AND completed_at < now() - interval '6 hours') \
            OR (status = 'dead'      AND completed_at < now() - interval '7 days')",
    )
    .execute(pool)
    .await?;
    Ok(res.rows_affected())
...
/// Enqueue a config_sync only if there isn't already a pending one for the
/// same (guild_id, role_id). The tick worker fans out a resync over every
/// role link every TICK_MINUTES; without this de-dupe a slow sync cycle
/// would pile identical jobs. Idempotent within the pending window.
pub async fn enqueue_config_sync_unique(
    pool: &PgPool,
    guild_id: &str,
    role_id: &str,
) -> Result<(), AppError> {
    sqlx::query(
        "WITH ins AS ( \
             INSERT INTO jobs (kind, payload, next_run_at) \
             SELECT 'config_sync', $1::jsonb, now() \
             WHERE NOT EXISTS ( \
                 SELECT 1 FROM jobs \
                 WHERE status = 'pending' AND kind = 'config_sync' \
                   AND payload->>'guild_id' = $2 AND payload->>'role_id' = $3 \
             ) \
             RETURNING 1 \
         ) \
         SELECT pg_notify('jobs_pending', '') FROM ins",
    )
    .bind(json!({ "guild_id": guild_id, "role_id": role_id }))
    .bind(guild_id)
    .bind(role_id)
    .execute(pool)
    .await?;
