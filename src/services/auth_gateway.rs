...
/// plugin is mounted under (`/birthday-role`) and the entry in the gateway's
/// plugin registry. The gateway uses this to scope the user's per-(plugin ×
/// server) opt-outs when filtering guild lists.
const PLUGIN_SLUG: &str = "birthday-role";
