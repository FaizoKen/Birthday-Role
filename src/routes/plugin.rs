...
//! save UX. POST /config is a no-op kept for contract compliance: it is
//! never called by iframe-mode plugins, but we still verify the token so a
//! leaked/stale call cannot ping us silently.
...
pub async fn register(
    ...
    // Ensure a guild_settings row exists so reads always succeed (Convention
    // 33 — server-wide settings live here, not on role_links).
    sqlx::query(
